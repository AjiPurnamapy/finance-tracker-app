"""
Subscription service — manage family subscription tiers.

MVP: Mock billing (no payment gateway). Upgrade/cancel operates
on subscription status and tier directly.
"""
...
async def upgrade_to_pro(user: User, db: AsyncSession) -> SubscriptionResponse:
    ...
    # M-3: Idempotency guard — prevent free subscription extension
    if sub.tier == SubscriptionTier.PRO and sub.status == SubscriptionStatus.ACTIVE:
        raise BadRequestException(
            code="ALREADY_PRO",
            message="Family ini sudah berlangganan PRO.",
        )
